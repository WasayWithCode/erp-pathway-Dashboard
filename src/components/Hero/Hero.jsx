...
const previewRows = [
  ...
  { label: "Inventory health", value: "74%", tone: "from-emerald-500 to-teal-400" },
  { label: "Hiring match", value: "91%", tone: "from-violet-500 to-fuchsia-400" },
];

const trustItems = ["SAP basics", "Oracle paths", "Odoo practice", "Dynamics careers"];
      ...
      <Container className="relative z-10 grid items-center gap-12 pb-20 lg:grid-cols-[1.02fr_0.98fr]">
        <div className="space-y-7">
          ...
          <div className="space-y-5">
            ...
            <p data-hero-item className="max-w-2xl text-lg leading-8 text-[#64748B]">
              ERP Pathway helps students understand enterprise workflows, choose modules, practice in dashboards,
              and move toward real consulting careers with confidence.
            </p>
          </div>
          ...
          <div data-hero-item className="flex flex-col gap-3 sm:flex-row">
            ...
            <Button to="/erp-simulator" variant="secondary" icon={<Icon name="dashboard" className="h-4 w-4" />}>
              Open simulator
            </Button>
          </div>
          ...
          <div data-hero-item className="grid max-w-xl grid-cols-3 gap-3">
            {[
              ...
              <div key={label} className="rounded-2xl border border-white/80 bg-white/70 p-4 shadow-[0_16px_40px_rgba(15,23,42,0.06)] backdrop-blur-xl">
                <p className="text-2xl font-semibold text-[#0F172A]">{value}</p>
                <p className="mt-1 text-xs font-semibold uppercase tracking-[0.16em] text-[#64748B]">{label}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div data-preview className="relative">
          <div data-float className="absolute -left-4 top-10 z-20 hidden rounded-2xl border border-white/80 bg-white/85 p-4 shadow-[0_24px_60px_rgba(37,99,235,0.16)] backdrop-blur-xl sm:block">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#64748B]">Match score</p>
            <p className="mt-1 text-3xl font-semibold text-[#0F172A]">92%</p>
          </div>
          <div data-float className="absolute -right-2 bottom-16 z-20 hidden rounded-2xl border border-white/80 bg-white/85 p-4 shadow-[0_24px_60px_rgba(6,182,212,0.16)] backdrop-blur-xl sm:block">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#64748B]">Live modules</p>
          ...
          <div className="relative overflow-hidden rounded-[2rem] border border-white/80 bg-white/80 p-4 shadow-[0_40px_120px_rgba(15,23,42,0.14)] backdrop-blur-2xl">
            <div className="rounded-[1.5rem] border border-slate-200/80 bg-slate-50/90 p-4">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#64748B]">ERP command center</p>
                  <h2 className="mt-2 text-xl font-semibold text-[#0F172A]">Learning operations</h2>
                </div>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-3">
                {["Finance", "Inventory", "People"].map((item, index) => (
                  <div key={item} className="rounded-2xl border border-slate-200 bg-white p-4">
                    <Icon name={index === 0 ? "finance" : index === 1 ? "inventory" : "people"} className="h-5 w-5 text-blue-600" />
                    <p className="mt-3 text-sm font-semibold text-[#0F172A]">{item}</p>
                    <p className="mt-1 text-xs text-[#64748B]">Workflow ready</p>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-4">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-semibold text-[#0F172A]">ERP readiness</p>
                  ...
                  {previewRows.map((row) => (
                    <div key={row.label}>
                      ...
                      <div className="h-2 overflow-hidden rounded-full bg-slate-100">
                        <div className={`h-full rounded-full bg-gradient-to-r ${row.tone}`} style={{ width: row.value }} />
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div data-hero-item className="mt-6 flex flex-wrap justify-center gap-2">
            {trustItems.map((item) => (
              <span key={item} className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-[#64748B]">
                {item}
              </span>
            ))}
          </div>
